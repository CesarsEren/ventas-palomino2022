	...
	$('#tblDocumentos').on('page-change.bs.table',function(e,number, size){
	 ...
	 $('#btnCargar').on('click',function(){
		 ...
		 $.ajax({
		        ...
		        url : "enviosunat",
		        data : {data:JSON.stringify(employees)},
		        dataType: 'json',
	 ...
	 $("#btneditar").on("click",function(){
		 $.ajax({
		    ...
		    url : "editdocumento?ruc="
		    	...
		    	+"&nro="+nrocode
		    	+"&tipo="+tipo,
		    //data : {ruc : $("#ruc").val(),razon: $("#razon").val(),dni: $('#dni').val(),nombre: $('#nombre').val(),nro,tipo,},
		    dataType: 'json',
		    success: function (res) {
		    if(res.msgserver == "SUCCESS")
		    {
	 ...
	 $('#btnVerificar').on('click',function(){
		 ...
		 $.ajax({
			 	type: "GET",
		        contentType: "application/json;charset=utf-8",
		        url : "estadodocumentossunat",
		        data : {empresa : $("#empresa").val(),fechaemision : $("#fechaDocumentos").val(),documentos : $('#documentos').val()},
		        dataType: 'json',
		        beforeSend: function (response) {
		 ...
		 $("#tblDocumentos").bootstrapTable('destroy').bootstrapTable(
			  {
				...
				responseHandler: function(res){
					...
					$("#btnVerificar").prop("disabled", res.documentos == 2 && res.total > 0 ? false : true);
...
function FormatterAction(value, row, index) {
	...
	var id = row['id'];
	var documento = row['documentoElectronico'];
	
	html.push('<button  class="btn btn-info btnfacturacion" onclick="openmodal(\''+servicio+'\','+id+',\''+documento+'\')"   data-nro="'+ id + '" data-servicio="'+ servicio + '" "> Editar </button>');
	return html.join('');
}
var nrocode;
var tipo;
var openmodal= function(servicio,nro,documento)
{
	tipo = servicio;
	nrocode = nro;
	var factura = documento.includes('F');
	//alert("asdasldoquerqw"+servicio+"---"+nro);

	$("#nrofactura").val(documento);
	var empresa= $("#empresa").val();
	
	$.ajax({
		 	type: "GET",
	        contentType: "application/json;charset=utf-8",
	        url : "Traerdocumento?nro="+nro+"&emp="+empresa+"&tipo="+servicio+"",
	        dataType: 'json',
	        beforeSend: function (response) {  
			},
	        success: function (res) {
	        	...
	        	$("#dni").val(res.dni.trim());
	        	$("#nombre").val(res.nombre.trim());
	        	
	        	
	        	if(factura){
	        		$("#secempresa").css("display","block");
	        		$("#ruc").val(res.ruc.trim());
	        		$("#razon").val(res.razon.trim());
	        	}else{
	        		$("#ruc").val("");
		        	$("#razon").val("");	        		
	        		$("#secempresa").css("display","none");
	        	}
	        },
	        error: function (e) { 
	        }
